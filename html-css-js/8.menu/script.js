...
function displayItems(items) {
    const arrOfArticle = items.map((item) => {
        return `<article>
            <img
                src=${item.img}
                alt=""
    ...
    container.innerHTML = arrOfArticle.join("");
}
// make all category as button
function btnfnc() {
    ...
    const btnsCreated = category.map((item) => {
        return `<button id=${item}>${item}</button>`;
    });
    btns.innerHTML = btnsCreated.join("");

    const allbtn = document.querySelectorAll(".btns button");
    allbtn.forEach((btn) => {
